...
class LoginController extends Controller
{
            ...
            try {
                ...
                if ($get_user) {
                    ...
                    if ($password == $decryptedPassword) {
                        ...
                        $request->session()->put('locations_all', $get_user['hotel_name']);

                        if ($get_user['user_role'] != 1) {
                            if (count(explode(',', $get_user['hotel_name'])) > 1) {
                                $final_location = Hotels::whereIn('id', explode(',', $get_user['hotel_name']))
                                    ->where('is_deleted', 0)
                                    ->get()->toArray();
                                $request->session()->put('location_for_user', $final_location);
                            } else {

                                $request->session()->put('location_selected', rtrim($get_user['hotel_name'], ','));
                                $final_location = Hotels::where('id', session('location_selected'))
                                    ->where('is_deleted', 0)
                                    ->first();
    ...
    public function getLocationSelectedAdmin(Request $request)
    {
        try {
            ...
            $final_location = Hotels::where('id', session('location_selected'))->where('is_deleted', '0')->first();
            $request->session()->put('location_selected_name', $final_location->hotel_name);
          
            $request->session()->put('location_selected_id', $final_location->id);
            // dd( $final_location->id);
            // die();
